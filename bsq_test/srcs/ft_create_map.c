...
/*   Updated: 2021/09/09 09:45:24 by jtian            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		**allocate_array(t_map_key key)
{
	...
	// +1 is reserved for the NULL pointer at the end
	while (i < key.row)
	{
		array[i] = (char *)malloc((key.column + 1) * sizeof(char));
		// +1 is reserved for the NULL in each lines
		i++;
	}
	if (array == NULL)
		return (NULL);
...
int			ft_open_file(char *filename)
{
	...
	//what's fd? why it can be -1? 1:stdout 0:stdin 2:stderr
	if (fd == -1)
	{
...
t_map_key	ft_read_file(int fd)//calculate the rows and columns
{
	int			r;
	int			c;
	char		ch;
